...
describe('Encode', () => {
	...
	tests.forEach((test) => {
		it(test.name, () => {
			if (test.shouldThrow) {
				expect(() => {
					Encode(test.encoding as Encodings, test.input);
				}).toThrow();
			} else {
				expect(Encode(test.encoding as Encodings, test.input)).toEqual(test.expected);
			}
		});
	});
});
...
describe('Decode', () => {
	...
	tests.forEach((test) => {
		it(test.name, () => {
			if (test.shouldThrow) {
				expect(() => {
					Decode(test.encoding as Encodings, test.input);
				}).toThrow();
			} else {
				expect(Decode(test.encoding as Encodings, test.input)).toEqual(test.expected);
			}
		});
	});
});
