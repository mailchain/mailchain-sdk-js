...
export async function createEnvelope(
	...
	const keyBundle = await createECDHKeyBundle(recipientIdentityKey, rand);
	const encrypter = PrivateKeyEncrypter.FromPrivateKey(ED25519PrivateKey.FromSeed(keyBundle.secret), rand);
	const encryptedMessageKey = await encrypter.Encrypt(EncodePrivateKey(messageKey.PrivateKey)); //TODO: look into encoding extended keys
