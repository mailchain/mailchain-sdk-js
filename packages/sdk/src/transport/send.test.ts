...
import { ConfigurationParameters, TransportApi, TransportApiFactory } from '../api';
import { Configuration } from '../api/configuration';
import { EncodeHexZeroX } from '../../../encoding/src/hex';
import { LookupResult } from '../identityKeys';
import { PayloadForRecipient, PayloadRecipientDelivery, Sender, LookupMessageResolver } from './send';
...
describe('Sending tests', () => {
	...
	test.each(tests)('$name', async (test) => {
		...
		if (test.shouldThrow) {
			expect(() => {
				const sender = new Sender(TransportApiFactory(apiConfig) as TransportApi, test.lookupMessageResolver);
				sender.sendPayload(test.payloads);
			}).toThrow();
		} else {
			const sender = new Sender(TransportApiFactory(apiConfig) as TransportApi, test.lookupMessageResolver);
			const result = await sender.sendPayload(test.payloads);
